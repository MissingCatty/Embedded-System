...
各个存储器统一编址，Flash的起始地址一般为`0x08000000`

| 存储器类型     | 特性                    | 主要用途                           |
| -------------- | ----------------------- | ---------------------------------- |
| Flash（闪存）  | 非易失，读快写慢        | 存储程序代码和常量                 |
| SRAM           | 易失，读写极快          | 存放运行时变量（堆、栈、全局变量） |
| Core Registers | 易失，CPU内部，速度最快 | 执行计算和程序流控制               |
| System Memory  | 非易失，只读            | 存放官方Bootloader，用于ISP下载    |
| Option Bytes   | 非易失，需特殊写入      | 配置底层硬件和安全特性             |

# 8.STM32烧录过程

当您使用烧录工具（如STM32CubeProgrammer或ST-Link）将 `.bin` 文件烧录到芯片时，工具所做的工作就是：

- 把 `.bin` 文件的**第0个字节**，写入到STM32的 `0x08000000` 地址。
- 把 `.bin` 文件的**第1个字节**，写入到 `0x08000001` 地址。
- 把 `.bin` 文件的**第2个字节**，写入到 `0x08000002` 地址。
- ...以此类推，直到文件的最后一个字节。

所以，`.bin` 文件的内容和芯片 `0x08000000` 开始的Flash内容是**一模一样、字节对字节的**。

这一切的源头，是您项目中的**链接器脚本 (`.ld` 文件)**。是这个脚本文件明确地告诉链接器：“请把中断向量表放在 `0x08000000`，然后紧跟着把代码段(`.text`)、只读数据段(`.rodata`)等内容都放到 `FLASH` 区域。”

链接器忠实地执行了这个“蓝图”，生成了 `.elf` 文件，`objcopy` 工具再从中提取出纯粹的二进制内容，形成了 `.bin` 文件。